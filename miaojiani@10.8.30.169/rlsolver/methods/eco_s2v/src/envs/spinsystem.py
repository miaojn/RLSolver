    ...
    def _reset_state(self, spins=None):
        ...
        if spins is None:
            if self.reversible_spins:
                ...
                # For MIS: 使用50/50均匀随机初始化，与MaxCut的reversible_spins=True一致
...
class SpinSystemUnbiased(SpinSystemBase):
    ...
    def calculate_mis(self, spins=None):
        """
        计算 Maximum Independent Set 的 QUBO 目标函数
        obj_QUBO = Σx_i - B * Σ_{(i,j)∈E} x_i * x_j
        
        其中:
        - x_i ∈ {0,1} 表示节点i是否被选中
        - B 是惩罚系数
        - x_i * x_j 在相邻节点同时选中时为1（违反约束）
        """
        if spins is None:
            spins = self._get_spins()
        else:
        ...
        # 目标项: Σx_i (选中的节点数)
        obj = np.sum(binary_spins)
        
        # 惩罚项: Σ_{(i,j)∈E} x_i * x_j (违反约束的边数)
        # 使用矩阵运算加速: 1/2 * x^T * A * x (除以2是因为每条边只计算一次)
        penalty_term = 0.5 * np.dot(binary_spins, np.dot(self.matrix, binary_spins))
        
        # QUBO 目标函数
        # B 设为 2.0: 意味着每个违反约束的边惩罚2个节点的收益
        B = 2.0
        obj_QUBO = obj - B * penalty_term
    ...
    def _calculate_mis_change(new_spins, matrix, action):
        """
        计算翻转节点后 QUBO 目标函数的增量变化
        Δobj_QUBO = Δ(Σx_i) - B * Δ(Σ_{(i,j)∈E} x_i * x_j)
        """
        # 将 {-1,1} 转为 {0,1}
        ...
        # 计算与action相连的已选中邻居数
        # 这等价于 Σ_{j:(action,j)∈E} x_j
        neighbors_selected = np.sum(binary_spins * matrix[action, :])
        
        B = 2.0  # 惩罚系数
        
        if action_value == 0:  # 准备选中节点action (0 -> 1)
            # Δobj = +1 (增加一个节点)
            delta_obj = 1.0
            # Δpenalty = neighbors_selected (新增这么多违规边)
            delta_penalty = neighbors_selected
            return delta_obj - B * delta_penalty
        else:  # 准备取消选中节点action (1 -> 0)
            # Δobj = -1 (减少一个节点)
            delta_obj = -1.0
            # Δpenalty = -neighbors_selected (减少这么多违规边)
            delta_penalty = -neighbors_selected
            return delta_obj - B * delta_penalty
    ...
    def _get_immeditate_mis_available_jit(spins, matrix):
        """
        计算每个节点翻转后的 QUBO 目标函数变化（即时奖励）
        对每个节点 i: Δobj_QUBO = Δx_i - B * Σ_{j:(i,j)∈E} x_j
        
        这个函数用于 Q-learning 中计算每个动作的即时奖励
        """
        # 将 {-1,1} 转为 {0,1}
        binary_spins = (spins + 1) / 2
        ...
        # 向量化计算: neighbors_selected[i] = Σ_j A_{ij} * x_j
        # 这表示节点i的已选中邻居数
        neighbors_selected = np.dot(matrix, binary_spins)
        
        # 对未选中的节点 (x_i = 0): Δobj_QUBO = +1 - B * neighbors_selected[i]
        # 对已选中的节点 (x_i = 1): Δobj_QUBO = -1 + B * neighbors_selected[i]
        rewards = np.where(
            binary_spins == 0,
            1.0 - B * neighbors_selected,  # 选中该节点的收益
            -1.0 + B * neighbors_selected  # 取消选中该节点的收益
        )
